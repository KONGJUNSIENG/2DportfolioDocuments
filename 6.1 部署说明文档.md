...
| 项目 | 值 |
|------|-----|
| 技术栈 | Next.js 15 + TypeScript Strict + Tailwind CSS v4 |
| 包管理 | pnpm ≥9.0.0 |
| 源码托管 | GitHub (KONGJUNSIENG/2D-portfolio) |
...
| 3 | Import Git Repository → 选择 `KONGJUNSIENG/2D-portfolio` | 确保 GitHub 已授权 Vercel |
...
| Header Rules | 无（默认） | — |
| Redirect Rules | 无（默认，由 middleware.ts 处理） | — |
...
- [ ] Case Study 详情页可访问（`/zh/projects/portfolio`）
- [ ] 立绘图片正常加载（Hero 区块）
- [ ] 字体渲染正确（日文/英文/中文各自字体）
- [ ] 响应式布局在各断点正常
- [ ] 动效按预期触发
...
| 监控项 | 状态 | 说明 |
|--------|------|------|
| Vercel Status Dashboard | 可用 | `vercel-status.com` |
...
| `pnpm run dev` 正常但 `pnpm run build` 失败 | `generateStaticParams` 数据缺失 | 确认 `src/data/` 下所有数据文件完整 |
...
本项目使用 `generateStaticParams` 预渲染 SSG 页面，**不受冷启动影响**。
...
| 节 | 关联文档 | 关系 |
|----|---------|------|
| 分支策略 | 完整技术规格书 §七 | 直接引用并展开 |
| CI 流水线 | 完整技术规格书 §七 | 直接引用并补充说明 |
| 部署架构 | 4.1 §八 | 约束来源，扩展项目具体配置 |
| 环境要求 | 4.1 附录 | 约束来源 |
| 技术选型 | 4.1 §一 | 约束来源 |
| 编码规范 | 5.1 | git commit 规范引用 |
| 开发任务 | 5.2 T7.7 | 最终部署任务引用 |
